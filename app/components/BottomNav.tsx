  ...
  return (
    <>
      ...
      <AnimatePresence>
        {showFab && (
          <>
            {/* Call Floating Button (Now on Left) */}
            <motion.a
              initial={{ scale: 0, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0, opacity: 0 }}
              href="tel:8918567688"
              className="fixed bottom-24 left-6 z-50 w-16 h-16 bg-brand-blue text-white flex items-center justify-center shadow-2xl md:hidden"
              style={{ borderRadius: '0px' }} // Sharp design
            >
              <Phone size={24} className="animate-pulse" />
            </motion.a>

            {/* WhatsApp Floating Button (Now on Right) */}
            <motion.a
              initial={{ scale: 0, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0, opacity: 0 }}
              href="https://wa.me/918918567688"
              target="_blank"
              className="fixed bottom-24 right-6 z-50 w-16 h-16 bg-[#25D366] text-white flex items-center justify-center shadow-2xl md:hidden"
              style={{ borderRadius: '0px' }} // Sharp design
            >
              <svg 
                viewBox="0 0 24 24" 
